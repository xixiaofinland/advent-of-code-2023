...
fn do_it(input: &str) -> usize {
    const BORDER: usize = 205;
    let mut v: Vec<usize> = vec![1; BORDER];

    for (i, data) in input.lines().enumerate() {
        let (win_cards, have_cards) = process_line(data);
        let wins = calculate(win_cards, have_cards);
        let wins = usize::try_from(wins).ok().unwrap();
        for _ in 0..v[i] {
            for j in (i + 1)..=(i + wins) {
                if j < BORDER {
                    v[j] += 1;
